...
extension ProtoTypeView {
    ...
    enum Modal {
        ...
        static func initial(_ i:Int = 0) -> RoundedRect/*CGRect*/{//init modal btn size
            ...
            let p:CGPoint = Grid.position(i,size)//Align.alignmentPoint(size, WinRect.size, Alignment.centerCenter, Alignment.centerCenter)
            let fillet:CGFloat = 50
            return RoundedRect(p,size,fillet)
            //return CGRect(p,size)
        }
        static func click(_ i:Int = 0) -> RoundedRect {//when modalBtn is pressed down
            let size:CGSize = Modal.initial(i).size * 0.75
            let p:CGPoint = Grid.position(i,size)//Align.alignmentPoint(size, WinRect.size, Alignment.centerCenter, Alignment.centerCenter)
            let fillet:CGFloat = Modal.initial(i).fillet * 0.75
            return RoundedRect(p,size,fillet)
        }
