...
class FastList4:Element,IList {
    ...
    func onListItemUpInside(_ buttonEvent:ButtonEvent) {
        ...
        selectedIdx = ArrayParser.first(pool, buttonEvent.origin as! Element, {$0 === $1.item}) ?? selectedIdx/*We extract the index by searching for the origin among the visibleItems, the view doesn't store the index it self, but the visibleItems store absolute indecies*/
