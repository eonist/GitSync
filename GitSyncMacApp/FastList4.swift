...
class FastList4:Element,IList {
    ...
    func onListItemUpInside(_ buttonEvent:ButtonEvent) {
        ...
        selectedIdx = ArrayParser.first(pool, buttonEvent.origin, {$0.item === $1}) ?? selectedIdx/*We extract the index by searching for the origin among the visibleItems, the view doesn't store the index it self, but the visibleItems store absolute indecies*/
