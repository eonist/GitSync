...
class ActivityList:List {
    override func mergeAt(objects: [Dictionary<String, String>], _ index: Int) {
        ...
        for object:Dictionary<String,String> in objects {// :TODO: use for i  
            let item:ActivityListItem = ActivityListItem(getWidth(), self.itemHeight ,object["repo-name"]!, object["contributor"]!,object["title"]!,object["description"]!,object["date"]!, false, self.lableContainer)
