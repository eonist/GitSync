...
class CommitCounter2 {
    ...
    func update(commitDB:CommitCountDB,repoList:[RepoItem],onComplete:@escaping ()->Void){//
       ...
       let group = DispatchGroup()
        ...
        repoList.forEach { repo in
            group.enter()
            bg.async {
                let commitCounts:[(date:YMD,commitCount:Int)] = self.update(commitDB:commitDB,repo:repo)
                main.async {
     ...
     */
    private func update(commitDB:CommitCountDB,repo:RepoItem) -> [(date:YMD,commitCount:Int)] {//⚠️️ commitDB may need to be inout
