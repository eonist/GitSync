...
class GitSync {
     ...
     * - Fixme: ⚠️️ Make the commit method try error based, to gain better knowledge of error
     */
     ...
     * - Fixme: ⚠️️ Contemplate implimenting a fetch call after the pull call, to update the status, whats the diff between git fetch and git remote update again?
    ...
    private static func initPush(_ repoItem: RepoItem, _ onPushComplete:@escaping PushComplete){
        MergeUtils.manualMerge(repoItem) {//🌵🌵🌵 commits, merges with promts, (this method also test if a merge is needed or not, and skips it if needed)
            ...
            let hasLocalCommits: Bool = GitAsserter.hasLocalCommits(repo.localPath, repoItem.branch)/*🌵🌵 TODO: maybe use GitAsserter's is_local_branch_ahead instead of this line*/
            if hasLocalCommits { // only push if there are commits to be pushed, hence the has_commited flag, we check if there are commits to be pushed, so we don't uneccacerly push if there are no local commits to be pushed, we may set the commit interval and push interval differently so commits may stack up until its ready to be pushed, read more about this in the projects own FAQ
