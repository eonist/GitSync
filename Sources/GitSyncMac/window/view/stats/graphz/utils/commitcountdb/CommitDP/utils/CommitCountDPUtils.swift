...
class CommitCountDPUtils{
    ...
    static func describeYear(commitDb:CommitCountDB){
        ...
        let commitCountDP = YearCommitDP(commitCount:yearCounts)
...
//        Swift.print("monthCounts: " + "\(monthCounts)")
        ...
        Utils.describeMonth(commitCountDP: commitCountDP)
    }
    /**
     * D
     */
...
private class Utils{
    ...
    static func describeMonth(commitCountDP:MonthCommitDP){
        ...
        for i in 0...commitCountDP.count{//⚠️️ we want to print the last month too
     ...
     * M
     */
    static func describeYear(commitCountDP:YearCommitDP){
        Swift.print("commitCountDP.count: " + "\(commitCountDP.count)")
