...
//    var prevMinY:CGFloat?//prevMinY to avoid calling start anim
    ...
    override var contentSize:CGSize {
        let width:CGFloat = GraphZ.config.itemSize.w * (graphArea.count-1)
        let contentSize = CGSize(width, height)//contentSize is -1 because the space inbetween the dots makes up 1 less
//        Swift.print("GraphScrollView5contentSize: " + "\(contentSize)")
        return contentSize
    }
    //
...
//        Swift.print("graphArea.count: " + "\(graphArea.count)")
    ...
    override func resolveSkin() {
        StyleManager.addStyle("ContainerView{fill:white;fill-alpha:1.0;}")//
//        Swift.print("🍊 ElementParser.stackString(self): " + "\(ElementParser.stackString(self))")
        super.resolveSkin()
    ...
    func frameTick() {
        ...
        let absX:CGFloat = abs(x).roundTo(100)
     ...
     * NOTE: scroll -> mover -> FrameTick -> tick
     */
    private func tick(){
//        Swift.print("tick(): ")
...
//        Swift.print("ratio: " + "\(ratio)")
    ...
    func createMoverGroup()->MoverGroup{
        Swift.print("createMoverGroup self.contentSize: \(self.contentSize)")
