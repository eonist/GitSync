...
extension CommitDB{
    ...
    func add(_ item:Commit){
        ...
        */
        ...
        let a = (sortedArr.count > 0 && )
        
        
        if(sortedArr.count > 0){
            let a = (closestIdx > sortedArr.count && sortedArr[closestIdx-1] != item)
            let b:Bool { return sortedArr[closestIdx] != item}
            if(a && b){
                
            }
        }
        if(a && sortedArr[closestIdx] != item){
            Swift.print("💚 insert")
